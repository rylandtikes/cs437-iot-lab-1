...
class TrafficObject(object):
    ...
    @staticmethod
    def is_close_by(obj, frame_height, min_height_pct=0.06):
        '''Defines object as close by if the bounding box is 6% of the frame height'''
        obj_height = (obj['bounding_box'][2] * frame_height) -(obj['bounding_box'][0] * frame_height)
...
class GreenTrafficLight(TrafficObject):
    ...
    def set_car_state(self, car_state):
        logging.debug('green light: continue driving')


class Person(TrafficObject):

    def set_car_state(self, car_state):
        logging.debug('pedestrian detected: stopping car')
